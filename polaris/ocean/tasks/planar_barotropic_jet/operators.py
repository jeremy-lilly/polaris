...
from polaris.ocean.tasks.planar_barotropic_jet.mat import inv_3x3
...
def trsk_mats(mesh):
    ...
    trsk.cell_del2_sums = trsk.cell_flux_sums * trsk.edge_grad_norm
...
def edge_grad_norm(mesh):
    ...
    # for non-periodic boundary in y
    # print(ivec[ivec < 0])
    # print(jvec[jvec < 0])
    # print(xvec)
...
def dual_lsqr_fxyz(mesh):
    ...
    Rinv, Rdet, matR, Sinv, Sdet, matS = dual_lsqr_mats(mesh)
...
def edge_lsqr_fxyz(mesh):
    ...
    Rinv, Rdet, matR, Sinv, Sdet, matS = edge_lsqr_mats(mesh)
